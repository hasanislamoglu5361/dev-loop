...
describe('FEATURE057 - Checkpoint Manager', () => {
  it('saves checkpoint state atomically and restores by turn', async () => {
    ...
    await manager.save({ loopId: 'loop-a', turn: 1, state: { step: 'coding', files: ['a.ts'] } });
  ...
  it('restores latest checkpoint using numeric turn order', async () => {
    ...
    await manager.save({ loopId: 'loop-a', turn: 2, state: { value: 'two' } });
    await manager.save({ loopId: 'loop-a', turn: 10, state: { value: 'ten' } });
  ...
  it('clears checkpoints for one loop without deleting other loops', async () => {
    ...
    await manager.save({ loopId: 'loop-a', turn: 1, state: {} });
    await manager.save({ loopId: 'loop-b', turn: 1, state: {} });
